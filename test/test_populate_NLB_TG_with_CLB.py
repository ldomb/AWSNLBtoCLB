...
def test_validate_environment_variable():
    from populate_NLB_TG_with_CLB import validate_environment_variable

    # Raise exception when MAX_LOOKUP_PER_INVOCATION <= 0
    with patch("populate_NLB_TG_with_CLB.LambdaEnv.MAX_LOOKUP_PER_INVOCATION", 0):
    ...
    with patch(
        "populate_NLB_TG_with_CLB.LambdaEnv.INVOCATIONS_BEFORE_DEREGISTRATION", 0
    ):
        with pytest.raises(ValueError) as e:
            expected_error_message = (
...
@patch("populate_NLB_TG_with_CLB.sys")
@patch("populate_NLB_TG_with_CLB.get_elb_ip_from_dns")
def test_get_ip_from_dns(mocked_get_elb_ip_from_dns, mocked_sys):
    from populate_NLB_TG_with_CLB import get_ip_from_dns
...
@patch("populate_NLB_TG_with_CLB.AwsServices")
@patch("populate_NLB_TG_with_CLB.logger", return_value=MagicMock())
def test_update_elb_ip_count_metric(mocked_logger, mocked_AwsServices):
    from populate_NLB_TG_with_CLB import update_elb_ip_count_metric
    ...
    with patch("populate_NLB_TG_with_CLB.LambdaEnv.CW_METRIC_FLAG_IP_COUNT", False):
...
def test_get_ip_from_previous_invocation(mocked_logger, mocked_AwsServices):
    from populate_NLB_TG_with_CLB import get_ip_from_previous_invocation
...
@patch("populate_NLB_TG_with_CLB.AwsServices", return_value=MagicMock())
@patch("populate_NLB_TG_with_CLB.logger", return_value=MagicMock())
def test_update_target_group(mocked_logger, mocked_AwsServices):
    from populate_NLB_TG_with_CLB import update_target_group
