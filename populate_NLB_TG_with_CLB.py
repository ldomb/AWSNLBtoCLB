...
"""
This function checks the DNS records for an internal Classic Load Balancer IP addresses.
It populates a Network Load Balancer's target group with Classic Load Balancer's IP addresses

WARNING: This function perform multiple DNS looks per each invocation. It is not guaranteed that all
Classic Load Balancer IP will be detected by a single invocation. However, the result converges
...
1. CLB_DNS_NAME - The full DNS name of the internal Classic Load Balancer
2. CLB_LISTENER - The traffic listener port of the internal Classic Load Balancer
...
def get_ip_from_dns():
    """
    Get CLB node IP address through DNS lookup. Exit if no IP found in the DNS
    :return: a set of ELB node IP addresses
    """
    ip_from_dns_set = get_elb_ip_from_dns(
        LambdaEnv.CLB_DNS_NAME, "A", LambdaEnv.MAX_LOOKUP_PER_INVOCATION
    )
    logger.info(
        f"ELB IPs from DNS lookup: {ip_from_dns_set}. Total IP count: {len(ip_from_dns_set)}"
    )

    # Check if there is CLB no IP in the DNS. If so, exit from the current Lambda invocation
    try:
        error_message = (
            f"No IP found from DNS for CLB - {LambdaEnv.CLB_DNS_NAME}. "
...
def update_target_group(
    ...
    if pending_registration_ip_set:
        pending_registration_ip_target_list = get_elb_ip_target_from_ip_list(
            list(pending_registration_ip_set), LambdaEnv.CLB_LISTENER
        )
    ...
    if pending_deregistration_ip_set:
        pending_deregistration_ip_target_list = get_elb_ip_target_from_ip_list(
            list(pending_deregistration_ip_set), LambdaEnv.CLB_LISTENER
        )
...
def lambda_handler(event, context):
    ...
    active_ip_from_dns_meta_data = {
        "LoadBalancerName": LambdaEnv.CLB_DNS_NAME,
    ...
    # ---- Step 3 -----
    # Get the active and pending CLB IPs that were collected from the previous invocation
    logger.info(
